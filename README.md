...
| | |
|---|---|
| 👥 多账号支持 | 账号 = 一个模拟器多开实例 + 一套独立配置（任务/扫荡/收件人）；活动设置为**全局**，所有账号共用；`run` 默认串行执行全部启用账号，失败隔离（一个账号挂不阻塞其余）；WebUI 顶部账号标签切换管理，旧版单账号配置自动迁移 |
...
- **配置页（按账号）**：模拟器类型（mumu/雷电/蓝叠等）与多开编号（每账号一个）、BAAS 目录/服务器、任务勾选、扫荡策略、收件人覆盖
- **活动页（全局）**：新活动自动推图开关、手动指定活动模块、活动截图资源补齐；`push_before_sweep`（默认开）在活动扫荡前先推图解锁任务（已 SSS 的关卡快速跳过）。活动设置为**全局**，所有账号共用，不按账号区分
...
- 活动推图依赖 BAAS 社区维护的活动模块（`module/activities/`，每个活动一个插件）。模块自动对应按优先级：手动配置 `activity.current_activity` > BAAS 记录的 `current_game_activity` > 活动标题关键词匹配；当 GameKee 标题为纯中文、未收录于内置别名表且 BAAS 无任何记录时无法自动对应，需手动在 WebUI「活动」页指定活动模块（全局设置）
...
WebUI「🎪 活动」页里的 **「补齐当前服活动截图资源」** 开关（默认关闭；活动设置为**全局**，所有账号共用，不按账号区分）：
