...
| 🎫 无活动时体力兜底 | 没有活动进行中时，可选把普通/困难图扫剩的体力交给**特别委托**（据点防御/信用回收，`clear_special_task_power`）：次数支持 `max`（BAAS 按当前剩余体力点满，不足一次自动停止），不会误购体力 |
...
| 📧 邮件通知 | 执行结果推送到邮箱（SMTP，支持 QQ/163/网易免费企业邮授权码）；可选「执行完成后回游戏主页截图并内联嵌入邮件」（adb 截帧，无需浏览器），收件人直接看到执行完成画面 |
| 🔄 失联自愈 | 任务执行期间模拟器/游戏失联（表现为读取体力失败、截图帧为空）时，自动重启一次模拟器并重新启动游戏后重试扫荡；仍失败则跳过扫荡继续执行，不中断整轮。可通过 `simulator.auto_restart_on_failure`（WebUI「模拟器 & BAAS」面板）关闭 |
| 📅 日程奖励最后领取 | `collect_reward` 在所有任务（含竞技场、扫荡）完成后执行，奖励档位全部解锁 |
| 🔁 多次执行 | 可配置执行轮数（WebUI 主操作条「轮数」/ 配置文件 `run_times` / CLI `--times`）：全部账号执行完一轮后**立即开始下一轮**（无间隔，跨轮复用 OCR 服务器）；汇总邮件按「第 N 轮」分节展示每轮结果，每个账号只附最后一轮的截图 |
...
python -m baas_plus.cli run --times 3        # 多次执行：全部账号执行一轮后立即开始下一轮，共 3 轮
...
> `run` 默认执行**全部启用账号**（每个账号独立模拟器实例，串行执行）；只跑单个账号在「添加参数」里加 `--account <账号名>`。想连续执行多轮（例如清完体力后立即再跑一轮），在 WebUI 主操作条设置「轮数」或在「添加参数」里加 `--times <N>`（如 `-m baas_plus.cli run --times 3`），每轮结束立即开始下一轮，汇总邮件按轮次分节展示。
...
- **主操作条**（执行按钮旁）：`轮数` 设置多次执行（全局，`run_times`），全部启用账号执行完一轮后立即开始下一轮
...
- **扫荡**：策略（`auto` 按体力算次数 / `fixed` 固定次数）、扫荡列表、活动关卡号；「没有活动时用剩余体力扫荡特别委托」开关与次数（`据点防御,信用回收`，数字或 `max`）
...
没有活动进行中时，可开启「用剩余体力扫荡特别委托」（默认关）：普通/困难图扫完后，
把剩余体力交给特别委托（据点防御/信用回收）。次数为 BAAS 原生 `special_task_times`
格式，两项对应两个委托，支持数字或 `max`（`max` = BAAS 按当前剩余体力点满扫荡，
不足一次时自动停止，不会误购体力）：

```
0,max    # 体力全部扫「信用回收」
max,0    # 体力全部扫「据点防御」
3,5      # 据点防御 3 次 + 信用回收 5 次
```

开启后勾选任务里的「特别委托」会自动改由扫荡阶段调度（同一轮只执行一次）。
